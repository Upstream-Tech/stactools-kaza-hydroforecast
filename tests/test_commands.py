...
from stactools.kaza_hydroforecast.commands import create_kazahydroforecast_command


class CommandsTest(CliTestCase):
    def create_subcommand_functions(self) -> List[Callable[[Group], Command]]:
        return [create_kazahydroforecast_command]
    ...
    def test_create_collection(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            result = self.run_command(
                f"kazahydroforecast create-collection {destination}"
            )
    ...
    def test_create_item(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            result = self.run_command(
                f"kazahydroforecast create-item {infile} {destination}"
            )
            assert result.exit_code == 0, "\n{}".format(result.output)
